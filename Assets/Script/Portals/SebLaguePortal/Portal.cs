...
public class Portal : MonoBehaviour
{
    ...
    {
        // Itera attraverso tutti i viaggiatori tracciati.
        for (int i = 0; i < trackedTravellers.Count; i++)
        {
            // Ottiene il viaggiatore corrente.
            PortalTraveller traveller = trackedTravellers[i];
            // Ottiene la trasformata del viaggiatore.
            Transform travellerT = traveller.transform;
            // Calcola la matrice di trasformazione dallo spazio del portale collegato allo spazio del portale corrente al mondo, quindi allo spazio del viaggiatore.
            var m = linkedPortal.transform.localToWorldMatrix * transform.worldToLocalMatrix * travellerT.localToWorldMatrix;

            // Calcola l'offset del viaggiatore rispetto al portale.
            Vector3 offsetFromPortal = travellerT.position - transform.position;
            // Determina da quale lato del portale il viaggiatore sta arrivando.
            int portalSide = System.Math.Sign(Vector3.Dot(offsetFromPortal, transform.right));
            // Determina da quale lato del portale il viaggiatore proveniva precedentemente.
            int portalSideOld = System.Math.Sign(Vector3.Dot(traveller.previousOffsetFromPortal, transform.right));
            Debug.Log(portalSide);
            Debug.Log(portalSideOld);

            // Teletrasporta il viaggiatore se ha attraversato da un lato del portale all'altro.
            if (portalSide != portalSideOld)
            {
                // Salva la posizione e la rotazione precedenti del viaggiatore.
                var positionOld = travellerT.position;
                var rotOld = travellerT.rotation;
                // Teletrasporta il viaggiatore e reimposta la sua posizione e rotazione precedenti.
                traveller.Teleport(transform, linkedPortal.transform, m.GetColumn(3), m.rotation);
                // Riporta il clone grafico nella sua posizione e rotazione precedenti.
                traveller.graphicsClone.transform.SetPositionAndRotation(positionOld, rotOld);
                // Richiama il metodo per notificare al portale collegato che un viaggiatore è entrato.
                linkedPortal.OnTravellerEnterPortal(traveller);
                // Rimuove il viaggiatore dalla lista dei viaggiatori tracciati.
                trackedTravellers.RemoveAt(i);
                // Decrementa l'indice per compensare la rimozione del viaggiatore.
                i--;
            }
            else
            {
                // Aggiorna la posizione e la rotazione del clone grafico del viaggiatore.
                traveller.graphicsClone.transform.SetPositionAndRotation(m.GetColumn(3), m.rotation);
                // Aggiorna l'offset precedente del viaggiatore.
