...
{
    static void MeshCutting(Plane plane, Mesh mesh, Transform Pos)
    {
        #region initialize variables
        // Set sub-mesh count to 2
        mesh.subMeshCount = 2;
        ...
        // Initialize variables for mesh cutting
        ...
        // Transform mesh vertices from world space to local space
        ...
        // If all vertices are on the negative side of the plane, clear mesh and return
        if (meshVerts.All(x => !isInPositiveSide(x)))
        {
            mesh.Clear();
            return;
        }
        // If all vertices are on the positive side of the plane, return without further processing
        if (meshVerts.All(x => isInPositiveSide(x)))
            return;
        #endregion

        #region cut mesh
        // Initialize array to keep track of vertices already added to the new mesh
        ...
        // Iterate through triangles of the first sub-mesh
        for (int i = 0; i < submesh1Triangles.Count; i += 3)
        {
            ...
            // Determine which vertices are inside and outside the cutting plane
            ...
            // Perform actions based on the number of vertices inside the cutting plane
        ...
        // Set flag to indicate subsequent triangles belong to the second sub-mesh
        isInSubMesh2 = true;

        // Iterate through triangles of the second sub-mesh
    ...
    public static void CutByPlanes(SliceThings obj, IEnumerable<Plane> planes, bool cutCollider = true)
    {
        ...
        if (!mesh)
            return;
        var t = obj.transform;
        //Debug.Log("name" + planes.Count() + " " + mesh, t);
        foreach (var plane in planes)
            MeshCutting(plane, mesh, t);
        mesh.Optimize();
        if (!cutCollider)
            return;
